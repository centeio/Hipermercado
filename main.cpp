	...
	do {
		...
		if (opcao == 1) {
			...
			do {
				...
				ProdutoFornecedor temp(nomeProduto,"",0,NULL,NULL);
				if (hipermercado->getProdutos().find(temp) == Hipermercado::ITEM_NOT_FOUND) {
					cout << "O produto que introduziu ṇo existe. Pretende introduzir outro nome (Y/N): " << flush;
					cin >> answer;
					while (tolower(answer) != 'n' && tolower(answer) != 'y') {
						cout << "Por favor introduza Y para sim e N para nao. " << flush;
						cin >> answer;
	...
	do {
		...
		if (opcao2 == 1) {
			...
			do {
				...
				if (fornecedor == NULL) {
					cout << "O fornecedor que introduziu ṇo existe. Prentende reintroduzir o nome(Y/N): " << flush;
					cin >> answer;
					while (tolower(answer) != 'n' && tolower(answer) != 'y') {
						cout << "Por favor introduza Y para sim e N para nao. " << flush;
						cin >> answer;
